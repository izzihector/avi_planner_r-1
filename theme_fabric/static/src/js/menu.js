odoo.define('web.Menu', function (require) {
    ...
    var Menu = Widget.extend({
        ...
        bind_menu: function() {
            var self = this;
            this.$menu_page_title = this.$el.parents().find('.current-page-title');
            this.$app_menu_wrapper = this.$el.parents().find('#fabric_app_menu');
        ...
        open_menu: function (id) {
            ...
            var page_title;
            if ($clicked_menu.data('is-primary')) {
                page_title = $clicked_menu.text().trim();
            } else {
                var primary_menu_id = $clicked_menu.closest('.oe_secondary_menu').data('menu-parent');
                var primary_menu = this.$app_menu_wrapper.find('a[data-menu=' + primary_menu_id + ']');
                page_title = primary_menu.text().trim()
            }
            if (page_title) {
                this.$menu_page_title.text(page_title);
            }
            ...
            var $toggle_sub_menu_button = $('#toggle_sub_menu')
            if ($sub_menu.children().length) {
                $toggle_sub_menu_button.addClass('displayable');
            } else {
                $toggle_sub_menu_button.removeClass('displayable');
            }
